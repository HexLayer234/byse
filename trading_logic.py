...
⚠️ ВАЖНО: Этот бот поддерживает ТОЛЬКО LONG (покупка) позиции!
- place_buy() открывает LONG позицию (покупка актива)
- place_sell() ЗАКРЫВАЕТ позицию (продажа актива)

🚫 НЕТ ПОДДЕРЖКИ SHORT ПОЗИЦИЙ:
Бот НЕ открывает SHORT (продажа в шорт) позиции самостоятельно.
Если SHORT позиция обнаружена на бирже:
1. Это произошло из-за ручного вмешательства
2. Бот предупредит через Telegram
3. Бот будет мониторить выход по стоп-лоссу/тейк-профиту
4. Расчет прибыли для SHORT адаптирован: profit = (entry - current) / entry

Для добавления поддержки SHORT нужно:
- Добавить функцию place_short() для открытия SHORT через create_market_sell_order
- Обновить логику входа в fully_autonomous_trader.py
- Добавить анализ сигналов для SHORT в smart_signals.py
"""
